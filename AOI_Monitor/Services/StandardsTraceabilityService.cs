...
public static class StandardsTraceabilityService
{
        ...
        string repoRoot)
    {
        ...
        Add(rows, "Project Spec", "High contrast plus green/red/yellow status semantics", "HMI-004", "Style guide plus HMI audit", CombineEvidence(RootDocPath(repoRoot, "DESIGN.md"), layout.Path), File.Exists(RootDocPath(repoRoot, "DESIGN.md")) ? (layout.Exists ? LayoutStatus(layout) : StandardsTraceabilityStatus.Partial) : StandardsTraceabilityStatus.Missing, StandardsTraceabilityBlockingLevel.Warning, "Color/status requirements are standards-aligned design rules, not a certification claim.");
        ...
        string repoRoot)
    {
        ...
        Add(rows, "ISO 9241-style HMI", "Self-descriptiveness: operator messages are plain language and simulated/not-validated states are labeled", "HMI-DESC", "Alarm tests, glossary, and HMI docs", CombineEvidence(TestPath(repoRoot, "PlainLanguageGlossaryTests.cs"), TestPath(repoRoot, "AlarmEventServiceTests.cs"), RootDocPath(repoRoot, "DESIGN.md")), TestsExist(repoRoot, "PlainLanguageGlossaryTests.cs", "AlarmEventServiceTests.cs") ? StandardsTraceabilityStatus.Satisfied : StandardsTraceabilityStatus.Partial, StandardsTraceabilityBlockingLevel.Warning, "Engineer/Admin detail may include technical data; operator text must remain plain language.");
        ...
        Add(rows, "ISO 9241-style HMI", "Expectation conformity: status colors, alarm severities, and readiness outcomes follow consistent meanings", "HMI-EXPECT", "Style guide and alarm model tests", CombineEvidence(RootDocPath(repoRoot, "DESIGN.md"), TestPath(repoRoot, "AlarmEventServiceTests.cs")), File.Exists(RootDocPath(repoRoot, "DESIGN.md")) && TestsExist(repoRoot, "AlarmEventServiceTests.cs") ? StandardsTraceabilityStatus.Satisfied : StandardsTraceabilityStatus.Partial, StandardsTraceabilityBlockingLevel.Info, "Green/amber/red outcomes are project conventions aligned to industrial HMI practice.");
        Add(rows, "ISO 9241-style HMI", "Learnability: user, deployment, and contributor checklists are maintained", "HMI-LEARN", "Documentation", CombineEvidence(DocPath(repoRoot, "USER_MANUAL.md"), RootDocPath(repoRoot, "CONTRIBUTING.md")), File.Exists(DocPath(repoRoot, "USER_MANUAL.md")) && File.Exists(RootDocPath(repoRoot, "CONTRIBUTING.md")) ? StandardsTraceabilityStatus.Satisfied : StandardsTraceabilityStatus.Partial, StandardsTraceabilityBlockingLevel.Info, "Documentation supports onboarding; it is not certification evidence by itself.");
        ...
        string repoRoot)
    {
        ...
        Add(rows, "ISO/IEC 25010-style Quality", "Maintainability: analyzers, PR gates, contributor checklist, and tests are enforced", "QUAL-MAINT", "Code-quality and PR evidence", CombineEvidence(codeQuality.Path, RootDocPath(repoRoot, "CONTRIBUTING.md"), Path.Combine(repoRoot, ".github", "pull_request_template.md")), codeQuality.IsPass && File.Exists(Path.Combine(repoRoot, ".github", "pull_request_template.md")) ? StandardsTraceabilityStatus.Satisfied : StandardsTraceabilityStatus.Partial, StandardsTraceabilityBlockingLevel.Warning, "Release builds use selected warnings-as-errors and script-based pattern checks.");
        Add(rows, "ISO/IEC 25010-style Quality", "Portability: release packaging scripts and deployment docs exist for Windows desktop delivery", "QUAL-PORT", "Publish script and deployment docs", CombineEvidence(Path.Combine(repoRoot, "Scripts", "publish.ps1"), DocPath(repoRoot, "DEPLOYMENT.md")), File.Exists(Path.Combine(repoRoot, "Scripts", "publish.ps1")) && File.Exists(DocPath(repoRoot, "DEPLOYMENT.md")) ? StandardsTraceabilityStatus.Satisfied : StandardsTraceabilityStatus.Partial, StandardsTraceabilityBlockingLevel.Info, "Portability is scoped to the supported Windows desktop package.");
    ...
    private static string RootDocPath(string repoRoot, string fileName)
