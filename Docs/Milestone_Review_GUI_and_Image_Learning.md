...
> **Status update (same day):** GUI defects G1-G6 from Section 3.1 and the demo-semantics items (amber/green demo chips to purple, font-preset px labeling, over-trimmed Home/alarm chips, synchronous learning-report export) were fixed and merged to `main` (PR #1). Learning-pipeline calibration defects L1, L2, and L3 (Section 4.3) are now fixed with regression tests: training without OK Validation retains the default threshold and is marked REVIEW instead of deploying a hair-trigger threshold (L1); the anomaly score is now threshold-independent and calibration runs against the disk-persisted model so calibrated false-call rates hold at inspection time (L2); and the tolerance map is persisted as lossless 16-bit grayscale (L3). Remaining open items: L4 (corrupt-image skip), L5 (sample-size confidence bounds), L6-L8, and the P1/P2 plan items.
