...
| L5 | **No sample-size statistics behind the false-call claim.** The sweep picks the lowest threshold meeting the target; with fewer than ~20 OK validation images the 5% target forces zero empirical false calls, so the threshold lands at max(OK score)+0.001 with zero margin. By order statistics, the next good board exceeds that with probability ~1/(N+1) (~17% at N=5) while the report prints "0.0%". | `ImageOnlyPcbLearningService.cs:256-259, 287-290` | The headline metric is statistically hollow on small sets. Fix: minimum OK-validation count before quoting a rate, confidence bounds (e.g., Clopper-Pearson upper bound), and/or a margin above max OK score; report "0 false calls in N images" rather than "0.0%". |
...
- **Trust ("properly discern defective vs good, near-zero false positives"): not yet.** L1-L3 mean the deployed threshold does not deliver the calibrated rate even on the calibration distribution; L5 means small-sample claims overstate. These are days-scale fixes, not redesigns.
...
2. Metric wording: quote "0 false calls in N validation images" with a minimum-N gate (L5) instead of "0.0%"; keep the threshold-sweep CSV as backup evidence.
