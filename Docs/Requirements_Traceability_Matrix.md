...
| AI-005 | Generate customer validation report. | AI Model Test prompt / Customer report prompt | Implemented | AI Model Test / Reports | `AOI_Monitor/Services/CustomerValidationReportService.cs`, `AOI_Monitor/Views/AIModelTestView.xaml.cs` | HTML and Markdown; PDF export available via `PdfExportService`. |
...
| 3D-006 | Live 3D camera profile inspection. | Stage 2 planned work | Planned | 3D Profile Viewer / Camera | `AOI_Monitor/Services/Profile3DSourceService.cs`, `Docs/ROADMAP.md` | 3D acceptance service and generic boundary exist, but real 3D acquisition remains not accepted. |
...
| S1-008 | Stage 1 exit evidence can be regenerated without manual app clicking. | Stage 1 exit evidence CLI prompt | Implemented | Tools / Services | `AOI_Monitor.Tools/Stage1ExitCommand.cs`, `AOI_Monitor/Services/Stage1ExitEvidenceService.cs`, `Docs/API_SPEC.md` | CLI runs preflight, batch validation, model acceptance when active ONNX is Ready, package export, export verification, and Stage 1 factory readiness. Prototype-only runs do not claim production model acceptance. |
| S1-009 | Provide image-only AI Training Setup for Golden / OK / Inspection image grouping. | Image-only PCB learning prompt | Implemented | AI / Models | `AOI_Monitor/Views/AiTrainingSetupView.xaml`, `AOI_Monitor/Services/AiTrainingSetupService.cs` | Engineer/Admin can import image groups and run learning; Operator can view only. No defect labels or bounding boxes required. |
| S1-010 | Persist Golden / OK Learning / OK Validation / Inspection / optional NG Validation grouping. | Image-only PCB learning prompt | Implemented | Data / Services | `AOI_Monitor/Models/ImageLearningModels.cs`, `AOI_Monitor/Data/AoiDatabase.cs`, `AOI_Monitor/Services/ImageLearningProjectService.cs` | Imported images store role, paths, SHA-256, dimensions, metadata, optional image-level truth, notes, and audit records. |
| S1-011 | Generate learned reference image from uploaded good samples. | Image-only PCB learning prompt | Implemented | Services / Reports | `AOI_Monitor/Services/ImageOnlyPcbLearningService.cs`, `AOI_Monitor/Services/ImageOnlyLearningReportService.cs` | `learned_reference.png` is Stage 1 image-only evidence, not live camera acceptance. |
| S1-012 | Generate tolerance map for normal visual variation. | Image-only PCB learning prompt | Implemented | Services / Reports | `AOI_Monitor/Services/ImageOnlyPcbLearningService.cs` | `tolerance_map.png` documents learned lighting/position/appearance variation. |
| S1-013 | Calibrate false calls using OK Validation images. | False-call calibration prompt | Implemented | Services / AI / Models | `AOI_Monitor/Services/ImageLearningFalseCallComparisonService.cs`, `AOI_Monitor/Services/ImageOnlyPcbLearningService.cs` | Reports must include OK Validation image count. NG Validation, when present, constrains possible escapes. |
| S1-014 | Export anomaly region overlays and heatmaps. | Visual evidence prompt | Implemented | Services / Reports | `AOI_Monitor/Services/ImageLearningOverlayExportService.cs` | Exports annotated overlays, heatmaps, side-by-side images, manifest, ExportHistory, and audit records. |
| S1-015 | Produce before/after false-call report. | False-call comparison prompt | Implemented | Services / Reports | `AOI_Monitor/Services/ImageLearningFalseCallComparisonService.cs` | Includes baseline vs learned metrics, threshold sweep, review burden, possible escapes if NG Validation exists, and limitation text if it does not. |
| S1-016 | Produce client visual learning report. | Client report prompt | Implemented | Services / Tools | `AOI_Monitor/Services/ImageOnlyLearningReportService.cs`, `AOI_Monitor.Tools/ClientImageLearningDemoCommand.cs`, `AOI_Monitor.Tools/LearnFromImagesCommand.cs` | `visual_learning_report.html/json` explains image groups, learned artifacts, false-call behavior, anomaly examples, recommended threshold, and evidence boundaries. |
...
| S2-005 | Integrate accepted real GigE/USB3 camera SDKs. | Stage 2 planned work | Planned | Camera | `Docs/ROADMAP.md`, `Docs/ARCHITECTURE.md` | No selected customer/vendor SDK adapter or real hardware acceptance evidence is committed. |
...
| S2-010 | Live 3D camera hardware integration. | 3D Profile prompt / Stage Mapping | Planned | 3D Profile Viewer | `Docs/ROADMAP.md` | Current 3D page is sample CSV mode plus an adapter boundary; no accepted real 3D sensor acquisition. |
...
| S3-005 | Control real robot, handler, conveyor, PLC, or safety hardware. | Stage 3 planned work | Planned | Robot / Handler | `Docs/ARCHITECTURE.md`, `Docs/ROADMAP.md` | No vendor SDK, PLC, or safety I/O implementation. |
| S3-006 | Map inspection coordinates to robot coordinates. | Stage 3 planned work | Planned | Calibration / Robot | `Docs/ROADMAP.md` | 2D board-mm display is planning evidence only, not robot transform validation. |
...
| S4-005 | MES authentication / SSO. | Roles prompt / Stage Mapping | Planned | Auth / MES | `Docs/ROADMAP.md` | Local role selector only. MES auth marked Stage 4 planned. |
| S4-006 | Production MES/ERP result upload/writeback. | Stage 4 planned work | Planned | MES / ERP | `Docs/ARCHITECTURE.md` | Mock mode must not be treated as production MES. |
...
| TR-008 | Documentation deliverables. | Documentation prompt | Implemented | Docs | `Docs/DEPLOYMENT.md`, `Docs/USER_MANUAL.md`, `Docs/ROADMAP.md`, this file | Documents distinguish implemented/planned boundaries. |
| TR-009 | 8-hour stability/soak-test mode. | Soak-test prompt | Partially Implemented | Log & Export / Tools | `AOI_Monitor/Views/ReportsView.xaml.cs`, `AOI_Monitor/Services/BatchSoakTestService.cs`, `AOI_Monitor.Tools/BatchSoakCommand.cs`, `Docs/VALIDATION.md` | In-app simulated soak plus a headless `batch-soak` harness (per-pass timing/memory/handles/SQLite growth, crash/stuck/memory-trend failure detection, HTML+JSON+CSV evidence) exist; an executed 8-hour run artifact is still required, and factory soak additionally needs real hardware sources. |
| TR-010 | 1-second visualization target tracking. | Performance prompt | Implemented | Main Inspection / AI Model Test / Reports / Tools | `AOI_Monitor/Models/WorkflowModels.cs`, `AOI_Monitor/Services/BenchmarkInspectionService.cs`, `AOI_Monitor.Tools/BenchmarkCommand.cs`, `Docs/VALIDATION.md` | Benchmark measures the golden-compare workload with p50/p90/p95/p99, cold-start vs steady-state split, CPU execution-provider labeling, engine/model config, and image dimensions; headless `benchmark` CLI for repeatable evidence. Hardware timing remains future work. |
...
| RP-006 | MES-based login. | Roles prompt / Stage 4 | Planned | Auth / MES | `Docs/ROADMAP.md` | Explicitly Stage 4 planned. |
